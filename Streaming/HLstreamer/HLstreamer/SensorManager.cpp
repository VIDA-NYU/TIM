...
void SensorManager::CreateStationaryFrameOfReference()
{
	auto locator = winrt::Windows::Perception::Spatial::SpatialLocator::GetDefault();
	auto locatability = locator.Locatability();
	if (locatability != winrt::Windows::Perception::Spatial::SpatialLocatability::PositionalTrackingActive) DebugPrint("PositionalTracking is not active!");

	auto referenceFrame = locator.CreateStationaryFrameOfReferenceAtCurrentLocation();
	m_worldCoordSystem = referenceFrame.CoordinateSystem();
}
