...
namespace YoloRuntime {
    ...
    class ObjectReader<TData> {
        ...
        public async Task ReadAsync(size) {
            SerializationHeader header = await ReadDeserialized<SerializationHeader>(headerLength);
            return await ReadDeserialized<TData>(header.Length);
        }
        ...
        private async Task<T> ReadDeserialized<T>(int bufferLength) {
            return MessagePackSerializer.Deserialize<T>(await ReadBytesAsync());
        }

        /* read a specified number of bytes from the stream */
        private async Task<byte[]> ReadBytesAsync(int bufferLength) {
            byte[] data = new byte[bufferLength];
            int bytesCount = await stream.ReadAsync(data, 0, bufferLength);
